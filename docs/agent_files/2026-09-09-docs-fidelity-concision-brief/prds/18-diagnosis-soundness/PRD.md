...
- No frontend code changes. `CarePlanView.tsx`, `buildPdfHtml.ts`, and `frontend/src/types/carePlan.ts` are not edited by this PRD — §4.6/§6 verify and state why none of the new fields need TS mirroring, and document one interaction with existing frontend behavior for the record.
...
**Should a fully-emptied `diagnosis.details` get different treatment, given an empty diagnosis card is itself a signal to the patient?** Considered and rejected as a special case. `CarePlanView.tsx:162` already guards the whole "What the Doctor Found" card on `result.diagnosis.details?.length > 0` — identical to how every other item list (`medications`, `tests`, etc.) already disappears silently when empty. If every `diagnosis.details` entry gets dropped for lacking a real citation, the card simply doesn't render, exactly as it already wouldn't for a note with no diagnosis facts at all. This PRD does not special-case that outcome, but flags as `[OPEN]` (§9) whether "the diagnosis card silently vanished" is actually the right signal versus an explicit "we couldn't confirm what was found" message — that is a product/copy decision for `CarePlanView.tsx`, out of this PRD's Non-Goals (no frontend changes), and is not blocking.
...
This exemption is written down here, not left implicit, because an undocumented exemption is the precise failure mode this entire PRD exists to fix. It is explicitly revisitable: flagged as `[OPEN]` in §9, conditioned on what PRD 05 §7.5's review-catch-rate study (once run) finds about the reviewer's miss rate on low-stakes, list-shaped content specifically.
...
**No TypeScript changes required.** Verified directly against `frontend/src/types/carePlan.ts`: the six existing `source_fact_ids` fields (on `Medication`, `Test`, `Procedure`, `OtherInstruction`, `FollowUp`, `WarningSign`) are correctly *absent* from every corresponding TS interface, because they are stripped server-side before the frontend ever receives a response (§4.5, and PRD 01 §6's original reasoning). The three new fields this PRD adds get the identical treatment — stripped at the same boundary, by the same function, before the wire response is built — so they must likewise be absent from `ReasonForVisit`'s TS interface (currently `{ reason: string; description: string }`, unchanged) and from `DiagnosisDetail`'s/`Diagnosis`'s TS interfaces (unchanged). No 08-owned file needs an edit.

**One pre-existing inconsistency discovered while verifying this, unrelated to this PRD's own changes, flagged rather than silently ignored:** `frontend/src/types/carePlan.ts`'s `CarePlanContent` interface declares `summary_fact_ids: number[]` as a required (non-optional) field — but per `_strip_internal_provenance`, this key is *always* popped before the API response, meaning the TS type is asserting a shape that will never actually be true at runtime (the key will always be absent, not present-with-an-empty-array). This predates this PRD (introduced alongside 01/08's original work) and this PRD does not touch `carePlan.ts`, so it is not fixed here — flagged as `[OPEN]` in §9 for 08 to clean up (likely: delete the field from the interface entirely, matching how the six item-level `source_fact_ids` fields are already correctly omitted).

**One behavioral interaction with existing frontend code, not a code change:** if `_verify_assembly`'s new diagnosis block (§4.3) drops every entry in `diagnosis.details`, `CarePlanView.tsx:162`'s existing guard (`result.diagnosis.details?.length > 0`) means the "What the Doctor Found" card simply does not render — identical behavior to today's handling of an empty `medications`/`tests`/etc. list. No code change is needed for this to work correctly, but see §9's `[OPEN]` item on whether silent disappearance is the right patient-facing signal for a dropped diagnosis specifically, as opposed to every other item type.
...
- `[RESOLVED: a fully-emptied diagnosis.details is not special-cased -- it silently hides the "What the Doctor Found" card via CarePlanView.tsx's existing length>0 guard, identical to how every other empty item list already behaves.]` — see the next `[OPEN]` item for the product question this raises but does not answer.
...
- `[OPEN]` — whether a fully-emptied `diagnosis.details` (after this PRD's guard drops every entry for lacking a citation) should render an explicit "we couldn't confirm the details of your diagnosis" message instead of silently disappearing, given the patient presumably knows they received *some* diagnosis at their visit and total silence could read as "nothing was found" rather than "we couldn't verify what was found." This is a product/copy decision for `CarePlanView.tsx`, out of this PRD's Non-Goals (no frontend changes) — flagged for the brief owner or a follow-up frontend PRD, not blocking this PRD's landing.
- `[OPEN]` — PRD 05 §7.5's LLM-review catch-rate evaluation protocol remains unrun. This PRD closes the deterministic gap for `reason_for_visit`/`diagnosis`/`changed_since_last_visit`, but the project-wide question of how often the non-fatal `review` step actually catches a fabrication it's supposed to catch — for these fields or the six already-covered ones — is still genuinely unmeasured. Not this PRD's scope to run (explicitly out of scope per the task framing), but directly relevant to how much weight the remaining LLM-only safety net (for `low_priority`, `questions`, and any residual risk on the now-covered fields) can actually bear.
- `[OPEN]` — whether §4.6's `low_priority` exemption should be revisited once PRD 05 §7.5's study (if run) shows the reviewer's miss rate on low-stakes, list-shaped content is materially worse than its miss rate on primary-section content. Recorded as a conditional trigger for reopening this decision, not a commitment to reopen it on any particular timeline.
- `[OPEN]` — `frontend/src/types/carePlan.ts`'s `CarePlanContent.summary_fact_ids: number[]` is declared as a required, always-present field even though `_strip_internal_provenance` always removes the key before the API response reaches the frontend — a pre-existing inconsistency discovered while verifying this PRD's own TS-mirroring question (S6), predating this PRD, not fixed here. Flagged for 08 as a small, independent cleanup (likely: delete the field from the interface, matching the six item-level `source_fact_ids` fields that are already correctly omitted).
