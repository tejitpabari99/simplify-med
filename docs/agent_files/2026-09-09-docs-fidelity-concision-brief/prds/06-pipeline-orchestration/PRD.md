...
Depends on: 01 (schema, `Constants.Enums` deletions, the `summary_fact_ids`/`source_fact_ids` internal-provenance fields — §4.6), 02 (`Unit`, `resolve_units_from_job_doc`), 03 (`ground()`), 04 (`assemble_and_render()`, deletion of the three old methods/prompts, and the citation-existence check that replaces the deleted `close_coverage()` — owned entirely by 04, enforced inline inside `_verify_assembly`, PRD 04 §4.4; see §4.10), 05 (`review()`, `correct()`), 07 (`curate_glossary_terms()`, `render_care_plan_text()`, `build_glossary_from_care_plan()`).
...
- Rewire `iter_steps`/`run()` to call `ground → assemble_and_render → review → correct`, in that order, followed by the deterministic close (glossary re-detection — the citation-existence check that replaces the deleted `close_coverage` is already enforced inline by 04's `assemble_and_render`, before `review`/`correct` ever run, so there is no separate call for `iter_steps` to make here), threading `units`/`facts`/`care_plan` through correctly, with glossary curation running on its own background thread from right after `detect_terms` until just before the deterministic close.
...
- Wire the deterministic close correctly: glossary re-detection runs after `correct()` as the pipeline's sole remaining deterministic close step, since the citation-existence check that used to need ordering against it (the old `close_coverage`/glossary question neither 05 nor 07 settled) is now enforced by 04 inline, before `assemble_and_render` ever returns — there is nothing left after `correct()` to order against glossary re-detection except glossary re-detection itself (§4.10).
...
- No prompt text, no LLM call bodies. `ground()`, `assemble_and_render()` (whose own inline post-check replaces the deleted `close_coverage()` — PRD 04 §4.4), `review()`, `correct()`, and `curate_glossary_terms()` are taken as given, exactly as 03/04/05/07 specify them.
...
def iter_steps(
    ...
    try:
        ...
        # Deterministic close (PRD 06 §4.10): glossary re-detection is the
        # only step left here. The citation-existence check that used to
        # need ordering against it — the direct replacement for the
        # deleted close_coverage — is already enforced by 04, inline
        # inside assemble_and_render, before review()/correct() even run
        # (PRD 04 §4.4). There is no separate call for iter_steps to make.
        try:
...
**The ledger (`list[Fact]`) is deliberately NOT added to either dataclass**, departing from PRD 01 §4.1.4's suggested shape ("`units: list[Unit]` / `ledger: list[Fact]` fields once 02/03 exist"). Verified: every consumer of `facts` (grounding's own verification, `assemble_and_render` — whose own inline post-check is the citation-existence property that replaced `close_coverage`, PRD 04 §4.4 — `review`, `correct`) runs *inside* `iter_steps`, before the single `PipelineRunResult` yield at the end — nothing downstream of that yield (the adapter, the worker, metrics, tests) ever needs the ledger. PRD 01 flagged the *intended shape* so a future author wouldn't reach for `CarePlan.raw` (deleted) as the ledger's carrier; it did not mandate the field exist regardless of a consumer. Adding an always-unread field is exactly the vestigial-surface pattern the brief argues against for `additional_info`/`RawArtifacts` elsewhere — not reproducing it here. Recorded `[RESOLVED]` in §9, since this is a visible departure from a suggestion (not a requirement) in an upstream PRD.
...
**`close_coverage` (05) is deleted outright, not just reordered.** 05's token-overlap completeness heuristic (`_COVERAGE_OVERLAP_THRESHOLD`, `_flatten_care_plan_text`/`_fact_anchor_tokens`) implemented the project's old "does every fact survive somewhere in the output" contract. The project-wide inversion (01 §9, 03 §9) retires that contract entirely: omission is no longer mechanically checkable now that the assembly LLM (04) is trusted to select what belongs in a patient-facing report under its AHRQ-guided prompt, and the only property left that's mechanically enforceable is **soundness** — every emitted item cites at least one real fact via `source_fact_ids`, and every cited id actually exists in the ledger.

**Settled: 04 owns this property entirely, enforced inline inside `assemble_and_render`'s `_verify_assembly` post-check (PRD 04 §4.4), before `review()`/`correct()` ever run.** This was open when this PRD was first drafted (which of 04 or 05 would land the replacement, and under what name, was explicitly left to the other agent's call); it is no longer open. 05 §4.7 confirms the same conclusion independently ("the citation-existence check belongs to 04, not this PRD") and explains why no second pass is needed after `correct()` either — the correction vocabulary never names `source_fact_ids` as a target, and the corrector's own diff check independently rejects any unnamed change to it (05 §4.6/§4.7). There is consequently **no standalone `check_citations`-style function anywhere in the pipeline, and no separate call for `iter_steps` to make** between `correct()` and glossary re-detection — the citation-existence property is already guaranteed true of `care_plan` by the time `assemble_and_render` first returns it, and stays true through `review`/`correct` by construction.

**What this leaves for the deterministic close:** glossary re-detection alone, run once after `correct()` (or after `assemble_and_render`, when `correct()` didn't run — either way, once `care_plan` has reached its final form). The original ordering question this section used to resolve — "does the citation check need to run before glossary re-detection, so `render_care_plan_text` sees whatever the check did to `care_plan`?" — dissolves along with the separate call: there is nothing left to order glossary re-detection against. §4.2's `iter_steps` body reflects this directly: no `check_citations(...)` line, just the `glossary_future.result(...)` / `build_glossary_from_care_plan(...)` sequence immediately after the `correct()` block.
...
`ReviewResult(...)` above deliberately omits a `coverage=` kwarg: `coverage` already defaults to `[]` on `ReviewResult` (05 §4.1), so omitting the kwarg constructs correctly. Nothing in `iter_steps` branches on `coverage`'s contents either — whether `correct()` runs is decided from `len(result.corrections) > 0` alone (05 §4.5), so `coverage` is observability-only from this PRD's perspective. There is no `check_citations`-style function to monkeypatch here at all — the citation-existence property is enforced entirely inside 04's `assemble_and_render` (already mocked above via `p.assemble_and_render`), not as a separate module-level call `iter_steps` invokes (§4.10).
...
- No `test_citation_check_runs_before_glossary_redetection`-style test exists in this suite: the citation-existence property is enforced entirely inside 04's `assemble_and_render` (PRD 04 §4.4, already exercised by `p.assemble_and_render`'s mock above and by 04's own test suite), not as a second, orderable `iter_steps` call — there is nothing here to order glossary re-detection against (§4.10).
...
- `[RESOLVED: close_coverage (05) is deleted outright, not replaced 1:1 -- its old completeness/omission contract is retired project-wide. The citation-existence property that replaces it is owned entirely by 04, enforced inline inside assemble_and_render's _verify_assembly post-check (PRD 04 §4.4), before review()/correct() ever run. There is no standalone check_citations-style function and no separate iter_steps call for it -- the deterministic close after correct() is glossary re-detection alone.]` — this PRD's first draft left open which of 04/05 would land the replacement and under what name; both 04 (§4.4) and 05 (§4.7) now independently confirm 04 owns it, so the ordering question this section used to resolve (citation check before or after glossary re-detection) no longer has a second call to order in the first place. See §4.10.
