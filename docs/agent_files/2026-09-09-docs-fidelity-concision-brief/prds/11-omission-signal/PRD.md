...
**Resolved:** log-only. If a future need arises for the aggregate to be queryable per-job outside of Cloud Logging (e.g., a support tool that looks up one job's stats by id), that is a new, distinct requirement this PRD does not have evidence for yet — flagged as `[OPEN]` in §9, not built speculatively here.
...
- `[OPEN: whether the real-world omission rate, once observed, should feed back into a revisit of the soundness-over-completeness inversion itself.]` — explicitly out of this PRD's scope (§3): this PRD makes the inversion falsifiable (§1's stated goal, quoting the source research bundle directly), it does not pre-judge what the evidence will show or commit to any particular response to it. A future decision, not a default.
- `[OPEN: whether a Cloud Logging-based metric/alerting policy should be built on jsonPayload.coverage_signal, and if so, at what rate threshold.]` — console/IaC configuration, not code; flagged in §8 as manual follow-up once a real-world baseline exists to threshold against. This PRD deliberately does not guess at a threshold with zero production data behind it.
- `[OPEN: whether a later, distinct requirement (e.g. a support/debugging tool that looks up one job's coverage stats by job id, outside of Cloud Logging) would justify revisiting §4.5's log-only decision.]` — no such requirement exists today; noted so a future PRD doesn't have to re-derive §4.5's reasoning from scratch if one arises, only re-weigh it against a concrete new need.
