...
- **Check the readability score on the first real end-to-end run for label-field skew.** `render_care_plan_text` (§4.4) deliberately includes short, non-sentence label fields (titles, dosage strings, timeframes) as their own paragraphs, on the starting assumption that this doesn't meaningfully distort the score (§9). Once 06 produces a real before/after readability score against actual pipeline output, spot-check whether the "after" score reads as visibly skewed by these short fields versus what a prose-only projection would have scored — and if so, narrow the field list to prose-only fields for the readability consumer specifically, per §9's resolution.
...
- `[RESOLVED: render_care_plan_text's projection includes every CarePlan string field listed in §4.4, short non-sentence label fields (titles, dosage strings, timeframes) included, with no narrowing for the readability consumer.]` — a deliberate starting choice, not an oversight: whether these short fields measurably skew the readability score's sentence-length statistics isn't resolvable without a real pipeline run scoring real before/after pairs (06's territory once wired), and guessing at a narrower field list now risks solving a problem that may not exist while reopening the "one projection or two" question §4.4 already settled. If a real end-to-end run shows the score visibly skewed by these label fields, the fix is narrowing the field list to prose-only fields for the readability consumer specifically — tracked as a concrete follow-up in §8, not left to be rediscovered.
