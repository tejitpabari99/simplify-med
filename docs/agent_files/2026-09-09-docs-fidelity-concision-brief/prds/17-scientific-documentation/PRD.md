...
Outline: (1) what the product does and does not try to do, (2) the "translate, don't interpret" boundary stated as a testable rule, (3) why fidelity and readability are in genuine tension, with the evidence, (4) a link table to the other four docs plus a pointer into 16's technical docs for mechanism.
...
- **Finalize the cross-reference convention with PRD 16's author** (§9) — this PRD proposes a concrete form but cannot unilaterally settle it, since 16 is being authored concurrently and its actual file names/anchors don't exist yet to link against.
...
- `[OPEN] — the exact cross-reference convention with PRD 16.` This PRD proposes a symmetric labeled-pointer form (a `Mechanism:` blockquote pointing into 16's docs from this family; a `Rationale:` blockquote pointing back from 16's docs into this family), demonstrated in §4.2's README sample ("For *how* the pipeline works... see `../pipeline.md`..."). Cannot be finalized unilaterally — 16 is being authored concurrently, its exact file names and anchor structure don't exist yet, and 16's own instructions ask it to propose the same convention from its side. Whoever lands second should reconcile the two proposals rather than each unilaterally picking one.
