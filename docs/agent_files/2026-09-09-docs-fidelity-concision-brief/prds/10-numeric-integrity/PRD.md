...
- **No change to `ground()` or `_verify_ledger`** (03's territory) — this PRD does not attempt to verify that grounding's `Fact.text` is itself numerically faithful to the source note; see §9 [OPEN] for why that's a real, adjacent, but out-of-scope gap.
...
- A standalone ordinal day-of-month with no adjacent month name ("on the 12th") is not specially excluded and could false-positive; not built defensively without evidence it occurs (§9 [OPEN]).
- If `assemble_and_render.txt` were ever changed to instruct numbered `steps[]` prefixes ("1. Take..."), that would introduce spurious digit tokens; it does not today (verified: no numbering instruction anywhere in `assemble_and_render.txt`), so not defended against pre-emptively (§9 [OPEN]).
...
- `[OPEN: a standalone ordinal day-of-month with no adjacent month name ("on the 12th") is not specially excluded and could false-positive.]` No evidence this occurs in the app's actual note distribution; revisit after the manual smoke test (§8) if observed.
- `[OPEN: numbered steps[] prefixes, if assemble_and_render.txt is ever changed to instruct them, would introduce spurious digit tokens this check doesn't defend against.]` Not built defensively today because `assemble_and_render.txt` contains no such instruction (verified by inspection) — revisit if that prompt changes.
- `[OPEN: Fact.text's own numeric fidelity to the source note is unchecked by any existing deterministic gate.]` `_verify_ledger`'s three checks (unit exists, quote verbatim, quote informative) verify `quote`, never that `text`'s numbers match `quote`'s numbers or the source. This PRD's check inherits whatever `Fact.text` says as ground truth; if grounding itself silently corrupts a number while paraphrasing, this check cannot catch it (only the [DEFERRED] item above would). Adjacent to this PRD's territory, not fixed here.
