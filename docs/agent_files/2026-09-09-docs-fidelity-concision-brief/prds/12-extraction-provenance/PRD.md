...
- `[OPEN: whether a future PRD should add a per-page OCR fallback for PDFs with an empty text layer (rather than the current silent-omission behavior), and if so, whether that changes SourceSpan's granularity from "effectively per-file" to "genuinely per-page" in practice.]` — out of this PRD's scope (§3); noted because §4.2's "per-file today" framing would need revisiting if that fallback is ever added, though the schema itself (per-(file, page) SourceSpan) already accommodates it without further change.
- `[OPEN: whether the fact-level aggregate (§4.8.2) should also be broken down by FactCategory, the way PRD 11's coverage-signal aggregate is (omitted_by_category).]` — not built here; the source research's own framing of this signal is "how much of this run rests on OCR," a single run-level question, not a per-category one the way omission naturally is (omission is meaningfully different per category — a missed warning sign matters differently than a missed follow-up date — while "was the evidence OCR'd" doesn't obviously carry the same category-dependent stakes). Left as `[OPEN]` rather than `[DEFERRED]` because it's a genuinely cheap add (the `facts` list already carries `.category`) if a future reader of the logs wants it; not added speculatively here since no consumer has asked for it.
- `[OPEN: LOG_EXTRA_KEYS merge coordination with PRD 10/11 if implemented as parallel, independent diffs against the same list rather than sequential tasks.]` — §4.9. Mechanically trivial (each addition is an independent list append) but flagged so implementation doesn't silently drop one PRD's entry if two of these land as concurrent edits to `utils/constants.py` without rebasing against each other.
