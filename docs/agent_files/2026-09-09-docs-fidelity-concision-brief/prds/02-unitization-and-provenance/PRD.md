...
- `[RESOLVED: PRD 09 (input-transport), a later independent follow-on, supersedes this PRD's transport decision -- input_provenance is not persisted as a JobDoc Firestore field after all; it moves, together with input_text, into one GCS object per job (PRD 09 §4.1/§4.11), and this PRD's §4.11-§4.13 document-size-budget accounting for sharing Firestore's byte ceiling with input_text is moot as a result. resolve_units_from_job_doc (§4.7 above) is deleted outright by PRD 09 §4.9, which has the worker call services.unitizer.unitize(text, provenance) directly instead of through that wrapper.]` — this PRD's *provenance design* is untouched and stands exactly as written: `SourceSpan`, `unitize`/`provenance_for_pasted_text`, the unit-granularity table (§4.2), and the OCR downscale ceiling change (§4.5) are all left standing by PRD 09 — see PRD 09 §4.14 for the full accounting of what it supersedes versus what it leaves standing.
