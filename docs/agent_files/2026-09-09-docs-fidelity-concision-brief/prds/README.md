...
**Status: all nine (10-18) are now implemented on this branch.** The "design-only, no code reflects them" claim below is superseded — it described this wave before `dev-code` ran against it; the grep-based verification it cites (PRD 16 §4.0: `extraction_method`, `NUMERACY`, `_UNIT_WORD_MAX_LENGTH` absent, `Medication.why` still `str = ""`) was true at that earlier point and is kept here as a record of what was checked, not as the current state. 10, 11, and 12 landed in an earlier session (see each folder's own `code-2026-09-14-0650.md`/`review-2026-09-14-0650.md`, `code-2026-09-14-0711.md`/`review-2026-09-14-0711.md`, and `12-extraction-provenance/code-2026-09-14-0611.md`/`review-2026-09-14-0611.md`); 18 also landed in an earlier session, but that session did not record a `code-*.md`/`review-*.md` pair in `18-diagnosis-soundness/` — its implementation and review happened, just not written up here. 13, 14, 15, 16, and 17 landed in this session: see `13-absent-value-contract/code-2026-09-14-1200.md`/`review-2026-09-14-1200.md`, `14-merge-provenance/code-2026-09-14-1200.md`/`review-2026-09-14-1200.md`, `15-design-doc-evidence-labeling/code-2026-09-14-1200.md` (docs-only, no dedicated review), `16-technical-documentation/code-2026-09-14-1200.md`/`review-2026-09-14-1200.md`, and `17-scientific-documentation/code-2026-09-14-1200.md`/`review-2026-09-14-1200.md`. **This wave has now been through a settling revision pass, same as 01-08/09.** Every `[OPEN]` item across PRDs 10-18 is resolved or explicitly `[DEFERRED]` — `grep -rn "\[OPEN" prds/` surfaces nothing but prose that references a past open item as history, and PRD 15's own discussion of the status-tag convention itself, across all nine files. The sections below record what was resolved, what was deferred and to where, and the `[DEFERRED]` items that remain real but are no longer open questions.
...
| 13 | absent-value-contract | `why: str \| None = None` on `Medication`/`Test`/`Procedure`/`OtherInstruction`, `""` normalized to `None` by a shared validator; deletes the "Not stated in your note." sentinel from both prompts; the one rendering fallback moves to `frontend/src/utils/nextSteps.ts`; adds one `why: null` example item to both `backend/tests/fixtures/care_plan.json` and `frontend/src/tests/fixtures/realCarePlanOutput.fixture.json` | 01, 04, 05, 08 | 532 |
...
| 15 | design-doc-evidence-labeling | Defines RF/DJ/PD evidence-provenance labels (orthogonal to the `[OPEN]`/`[RESOLVED]`/`[DEFERRED]` status-tag axis), an inline-tag convention, and seeds `docs/uncalibrated-constants.md` with 6 real constants; retrofits RF/DJ/PD tags onto `comparison-drive-research-bundle.v1.md`'s §5 recommendations table and evidence-heavy sections — its one named exception to touching no existing doc — no other code, schema, or test | — | 161 |
| 16 | technical-documentation | A line-by-line audit finds 4 of 7 `docs/` files materially false about the current four-call pipeline; specifies a rewritten `pipeline.md`/`architecture.md`/`data-and-privacy.md`, a new `flow.md` (mermaid sequence diagram), `error-taxonomy.md`, and `testing.md`; documents the **post-01-09** state now, not the post-10-18 state; creates `docs/uncalibrated-constants.md`, seeded verbatim from PRD 15 §4.3 | 01-09 (landed), aware of 10-15 (written), 15 specifically for vocabulary/register and the constants register's content | 255 |
...
17 scientific-documentation (needs 15 directly + 16 directly; the two
   families' own cross-reference convention is settled jointly -- see
   Cross-PRD couplings)
```
...
2. **10, 11, 12 — the three independent, log-only leaf PRDs.** Each depends only on already-landed wave-1 PRDs. Land the code in any relative order, but land the `Constants.Observability.LOG_EXTRA_KEYS` edits themselves as sequential diffs, in the order 10 → 11 → 12, each merging its own append into the list as it stands rather than a wholesale rewrite (11 adds `coverage_signal`, 12 adds `extraction_signal`/`extraction_signal_facts`; **10 does not touch this list at all**, so its slot in the sequence is a no-op — see Cross-PRD couplings for why the merge discipline still matters).
...
4. **14 — merge-provenance.** Lands after 13, matching the order 18 itself declares. Because 14 ends up making zero `care_plan.py` edits (§4.1's rejection), the actual landing-order risk between 13 and 14 is low regardless — this keeps the dependency chain 18 declares intact rather than for any technical necessity. Note the soft, non-blocking flip side: 14's `merge_candidate_signal` cannot see a diagnosis-category merge until 18 lands (14 §9, resolved) — landing 14 here does not require waiting on 18, it just means that specific observability gap stays open until step 5 below closes it.
5. **18 — diagnosis-soundness.** Depends explicitly on 13 and 14. Also land it after 10: the two share an undeclared seam at the tail of `_verify_assembly` (Cross-PRD couplings), and since 10 has no dependency on 18 and was conceived first, land 10's `_check_numeric_parity` call first and rebase 18's new diagnosis block after it, not the reverse. Landing 18 here also closes 14's diagnosis-merge observability gap noted in step 4, as a side effect of the `source_fact_ids` fields this PRD adds.
...
| Coupling | Where decided | Reaches into |
|---|---|---|
| `Constants.Observability.LOG_EXTRA_KEYS` gains three independent appends to the same list: `coverage_signal`, `extraction_signal`/`extraction_signal_facts`, `merge_candidate_signal` | 11 (§4.3), 12 (§4.9), 14 (§4.4) | `backend/utils/constants.py` — **resolved (PRD 12 §9): the 10/11/12 additions land as sequential diffs, in the order 10 → 11 → 12, not concurrent parallel edits**, each merging its addition into the list as it stands at that point rather than overwriting it wholesale from its own PRD's snippet. 14's later `merge_candidate_signal` append (landing after 13/18 per the recommended order above) follows the identical merge-not-overwrite discipline. Note: **10 does not add an entry to this list** — confirmed by direct reading of PRD 10 §4.4 (it logs via plain string interpolation, no `extra=`) and by PRD 12 §4.9's own cross-check — despite this batch's initial framing assuming it might |
...
| `docs/uncalibrated-constants.md` authorship | 15 §4.3 (specifies the content, seeded verbatim), 16 §4/§9 (creates the file and links to it from `pipeline.md`), 17 §9 (only links to it, per the confirmed split) | **Resolved, jointly, across all three** — 16 creates the file as its own `dev-tasks`/`dev-code` deliverable, 15 supplies the content verbatim, 17 never authors it, only links |
| The 16↔17 documentation cross-reference convention | 16 §3, 17 §9 — **reconciled**: inline (`"— see X for why"` / `"— see X for the exact mechanism"`) is the default for a claim-level pointer; 17's blockquote form survives, narrowed to a document/section-level orientation signpost (17 §4.2's README sample is that case), never mid-prose | **Resolved in both PRDs.** Not a conflict once the two proposals are read as operating at different granularities rather than competing for the same job — 16 §3 and 17 §9 now both record the identical decision |
...
### Consolidated dispositions (10-18)

**The standing cross-batch fact carried into this wave.** PRD 05 §7.5's injected-error reviewer catch-rate protocol is specified but still unrun (reverified here: no `manual_reviewer_catch_rate.py` or `catch_rate`-named file exists anywhere in the repo). Every non-fatal, LLM-only backstop this wave still leans on inherits the same caveat — the reviewer's real-world value is asserted, not measured. Cited directly by 10 §1, 11 §1/§8, and 18 §9; 18 further ties its own `low_priority` exemption and its residual reliance on review for the newly-covered fields to whatever this protocol eventually shows. Not resolved by this batch — genuinely unmeasured, tracked under PRD 05, not gating anything here.

**Three findings, discovered during this batch, each now carrying a recorded disposition:**

1. **A PDF page with no text layer is silently dropped, never OCR'd.** Found while writing PRD 12 (§3, §4.2). `extract_pages_from_pdf` has no per-page OCR fallback — a scanned page inside an otherwise-native PDF vanishes from the document with no warning and no trace, and nothing downstream can detect it, because the content was never in the ledger to check against. `[DEFERRED]` (PRD 12 §9): no PRD owns building the fallback; `extraction_method`'s per-`(file, page)` `SourceSpan` placement leaves room for it without a further schema change, but PRD 12 does not build one — explicitly out of its scope (12 only tags what *was* extracted).
2. **`Fact.text`'s own numeric fidelity to the source is unchecked.** Found while writing PRD 10 (§4.3). Grounding can mangle a number while paraphrasing into `Fact.text`, and PRD 10's numeric-parity check deliberately validates a rendered field against `Fact.text`, not against the original note or `quote_for()` (§4.3's three-reason argument for that choice) — so a number grounding itself corrupted is invisible to this check by construction. `[DEFERRED]` (PRD 10 §9), homed to **PRD 03's `_verify_ledger`** — the natural place to extend a `text`-vs-`quote` numeric consistency check, if it's ever built.
3. **`frontend/src/types/carePlan.ts` types `CarePlanContent.summary_fact_ids` as required**, but `_strip_internal_provenance` always strips it server-side, so it is always absent at runtime. Found while writing PRD 18 (§6, §9) — a pre-existing type bug predating this batch entirely, **assigned to PRD 08** (which owns `carePlan.ts`) to delete the field, matching how the six item-level `source_fact_ids` fields are already correctly omitted. PRD 08 §9 carries a short forward-pointer recording the hand-off.

**Per-PRD dispositions, not already covered above or in Cross-PRD couplings:**

- **10** — `[RESOLVED]`. A standalone ordinal day-of-month ("on the 12th") isn't excluded from the numeric tokenizer and could false-positive: accepted as-is, no evidence it occurs in the app's note distribution, revisit only if the §8 manual smoke test ever shows it. A future numbered `steps[]` prefix in `assemble_and_render.txt` would introduce spurious digit tokens: not defended against pre-emptively, since no such instruction exists today — whoever changes that prompt in the future owns updating this check.
- **11** — `[RESOLVED]`: whether a real-world omission rate should feed back into revisiting the soundness-over-completeness inversion is explicitly out of this PRD's scope — it makes the inversion falsifiable, it doesn't pre-judge the answer. `[DEFERRED]`: a Cloud Logging-based metric/alert on `coverage_signal.rate` — console/IaC work, needs a production baseline first (§8).
- **12** — `[RESOLVED]`: the fact-level OCR aggregate (§4.8.2) stays run-level, not broken down by `FactCategory` — "how much of this run rests on OCR" is a single run-level question, unlike omission.
- **13** — `[RESOLVED]`: `test_pipeline_review.py`'s `_care_plan_with_two_medications()` fixture keeps its hard-coded "Not stated in your note." sentinel text exactly as-is — it remains a legal `why` string and no test's correctness depends on it being that exact sentence; touching a shared fixture for cosmetic reasons alone is the kind of unforced change this PRD otherwise avoids.
- **14** — `[RESOLVED]`. Diagnosis-category merges had no `source_fact_ids`-shaped signal at all when 14 was written (§9). **Closed by PRD 18**, written later in the same batch, which adds `source_fact_ids` to `DiagnosisDetail`/`ReasonForVisit` (§4.2) — 14 and 18 now cross-reference each other on this point (14's header/§9, 18's header/§5), recorded as a soft, non-blocking ordering relationship: 14 is fully implementable on its own; 18 only extends what its free merge signal can see. `[DEFERRED]`: a log-based metric/alert on `merge_candidate_signal`, matching 11's identical deferral for `coverage_signal`.
- **16** — `[RESOLVED]`: exact target filenames for 17's doc family are `docs/science/{README,design-rationale,good-summary-conformance,evidence-map,research-corpus}.md` (17 §4.1); the cross-reference convention with 17 (see Cross-PRD couplings); `docs/testing.md` stays backend-focused with a short frontend pointer, rather than matching the backend's depth; root `README.md`'s diagram stays terse ASCII, not replaced by a reduced `flow.md` mermaid diagram; and this PRD creates `docs/uncalibrated-constants.md` itself, seeded verbatim from PRD 15 §4.3.
- **17** — `[RESOLVED]`: no public arXiv/DOI identifiers were confirmed for the four Drive-only third-party papers (AgenticSum, Asgari et al., Croxford et al., Fact-Controlled Diagnosis) — the Drive copy is the source of record, the durability risk is disclosed rather than hidden, and finding a public identifier later is a welcome but unowned improvement, not a blocking question. `[RESOLVED]`: no separate re-verification pass for `good-summary-conformance.md` is scheduled once 10-18 land as code — PRD 17 §7's own maintenance-ownership rule already binds whichever future PRD changes a cited behavior to update that row itself.
- **18** — `[RESOLVED]`: a fully-emptied `diagnosis.details` no longer silently disappears — the "What the Doctor Found" card renders a "We couldn't confirm the specific findings from your note." fallback whenever there is other evidence of a visit (a populated `reason_for_visit`, or a non-empty `changed_since_last_visit`); with no evidence of a visit at all, the card still does not render. A bounded frontend change, specified in §6 (`CarePlanView.tsx`, `buildPdfHtml.ts`), overriding this PRD's former "no frontend changes" Non-Goal. `[DEFERRED]`: the `low_priority` exemption (§4.6) stands as specified, not revisited now — conditionally revisitable if PRD 05 §7.5's catch-rate study is ever run and shows a materially worse miss rate on low-stakes, list-shaped content specifically.
...
- **15** — Retrofit the comparison doc (`comparison-drive-research-bundle.v1.md`) with RF/DJ/PD tags on its §5 recommendations table and evidence-heavy sections — decided, not a choice left open; a `dev-tasks`/`dev-code` task against that one file. Add the one-line pointer to `design-agent-prompt-template.md` — also decided; **a machine-level skill file outside this repository**, so per this machine's own orchestration rules that edit is routed through the `update-config` skill or the owner directly, never a `simplify-med` sub-agent.
- **16** — Confirm the sequencing decision (document the post-01-09 state now, not post-10-18). Confirm the file-split decisions (`error-taxonomy.md`, `testing.md`, `flow.md` as new files, versus folding one or more back into `pipeline.md`/`architecture.md`). The §3 cross-reference convention is now reconciled with 17 (see Cross-PRD couplings) — nothing left to sanity-check here. Approve before `dev-tasks` runs; `dev-tasks`/`dev-code` also creates `docs/uncalibrated-constants.md` here, seeded verbatim from PRD 15 §4.3, as one of this PRD's own deliverables.
- **17** — Decide the `Medical device?` column's actual classification (a regulatory/legal, owner-only judgment — every row already carries an explicit "Not assessed" marker rather than a blank). Approve the vendoring decision (§4.6 — condensing, not verbatim-reproducing except the criteria doc's own table, the team's own memos into the repo). Have `dev-tasks`/`dev-code` author the five files once approved. Optional, unowned: a public arXiv/DOI identifier for the four Drive-only third-party papers (AgenticSum, Asgari et al., Croxford et al., Fact-Controlled Diagnosis) would remove a durability risk but isn't required to ship.
...
- 15, 16, and 17 are design-only: none of them rewrites a doc, edits a prompt, or touches application code themselves (16 §4.6 specifies exactly one new, narrowly-scoped test file as its sole exception, not yet written; 15 §4.4 similarly names one exception — RF/DJ/PD tags retrofit onto the comparison doc — as a `dev-tasks`/`dev-code` deliverable, not a write 15 makes itself) — `dev-tasks`/`dev-code` is the next step for all three once approved.
