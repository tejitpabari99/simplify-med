...
- **14** — `[RESOLVED]`. Diagnosis-category merges had no `source_fact_ids`-shaped signal at all when 14 was written (§9). **Closed by PRD 18**, written later in the same batch, which adds `source_fact_ids` to `DiagnosisDetail`/`ReasonForVisit` (§4.2) — 14 and 18 now cross-reference each other on this point (14's header/§9, 18's header/§5), recorded as a soft, non-blocking ordering relationship: 14 is fully implementable on its own; 18 only extends what its free merge signal can see.
- **16** — exact target filenames for 17's doc family, `[RESOLVED]`: `docs/science/{README,design-rationale,good-summary-conformance,evidence-map,research-corpus}.md` (17 §4.1), and the cross-reference *convention itself* is now also `[RESOLVED]` jointly (see Cross-PRD couplings) — inline as the default, 17's blockquote reserved for document-level orientation signposts. Still open: whether `testing.md` should document the frontend's Vitest suite in the same depth as the backend's test layers.
