...
Depended on by: nothing directly (blocking). Shares two files with 13 (absent-value-contract) — `care_plan.py` and `assemble_and_render.txt` — see §4.7 for the resolved seam. 16/17 (technical/scientific documentation) will document whatever this PRD lands on. Related: 18 (diagnosis-soundness) — a soft, non-blocking ordering relationship: 18 adds `source_fact_ids` to `DiagnosisDetail`/`ReasonForVisit`, which extends this PRD's free `len(source_fact_ids) > 1` merge signal to diagnosis-category merges once 18 lands (§9).
...
- No change to `Diagnosis`/`DiagnosisDetail` — they carry no `source_fact_ids` field today (confirmed in §4.1 and recorded as `[RESOLVED]` in §9, closed by PRD 18), and adding one is a schema change to a family PRD 01 already settled, not something this PRD's scope (the six `source_fact_ids`-bearing item models) authorizes. (PRD 18 makes exactly this schema change, separately — see §9.)
...
- **[RESOLVED: PRD 18 (diagnosis-soundness) adds `source_fact_ids` to `DiagnosisDetail` and `ReasonForVisit`, so the free `len(source_fact_ids) > 1` signal this PRD relies on extends to diagnosis-category merges once 18 lands.]** `Diagnosis`/`DiagnosisDetail` carry no such field today (§4.1), so the free signal this PRD relies on for the six item models is structurally unavailable for the very category the MERGE paragraph's own worked example uses. This PRD does not itself propose extending source-fact provenance to `Diagnosis` — that would have been a schema change to a family PRD 01 already settled, and a bigger ask than this "cost-reduced, low priority" sub-project's scope. PRD 18, written later in the same batch for an unrelated reason (closing a citation-existence blind spot on `reason_for_visit`/`diagnosis`), makes exactly that schema change as a side effect, and its own §9/header record the reverse pointer. **This is a soft ordering dependency, not a blocking one**: this PRD is fully implementable and correct on its own — `merged: bool` is still rejected, the prompt hardening and regression fixtures in §4.2/§4.3 stand regardless — it simply cannot observe a diagnosis-category merge via the free signal until 18's fields exist. No landing-order requirement is introduced by this resolution; 18 already lists 14 as a dependency for a separate reason (§4.7/header), so the observability gap closes automatically once the batch's own recommended order is followed.
- **[DEFERRED: a log-based metric/alert on `multi_fact_counts`/`merge_candidate_signal`.]** Would require console/IaC work; deliberately not built here, matching PRD 11 §7.3's identical deferral for `coverage_signal`.
