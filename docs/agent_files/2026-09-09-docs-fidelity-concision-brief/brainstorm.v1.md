...
| Risk | Cheapest test |
|---|---|
| **Extraction recall on raw clinical text may be worse than on pre-simplified text.** This is the single axis on which the inverted architecture could lose, and no published work measures it — a real gap in the literature, not just in our knowledge. | The gating experiment: 30–50 notes, run the grounding prompt on raw OCR'd text and on the current simplify+clarify output, score recall against a hand-annotated list of atomic facts. About a day of annotation plus two runs. Feeding the abbreviation list into grounding (3.3) is the mitigation if recall is the problem. |
